...
from src.hotkeys import (
    apply_shortcut, WM_TEMPLATES, DEFAULT_SHORTCUT, SHORTCUT_MARKER,
)
...
class SettingsWindow(QDialog):
    ...
    def _on_save_shortcut(self):
        ...
        cb = None
        parent = self.parent()
        if parent is not None and hasattr(parent, 'fb_app'):
            fb_app = getattr(parent, 'fb_app', None)
            if fb_app is not None and hasattr(fb_app, '_on_tray_show'):
                cb = fb_app._on_tray_show
